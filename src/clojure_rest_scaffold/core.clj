(ns clojure-rest-scaffold.core
  (:require [reitit.ring :as ring]
            ...
            [clojure-rest-scaffold.middleware :as middleware])
  (:gen-class))
  ...
  (ring/ring-handler
   (ring/router
    [["/swagger.json"
     ...
     ["/hello"
      {:swagger {:tags ["Hello Test"]}}

      ["/world"
       {:get {:summary "Test Api Aja"
              :handler (fn [req]
     ...
     ["/math"
      ...
      ["/test"
       {:get {:summary "test"
              :parameters {:query {:nama string?}}
              :handler (fn [{{{:keys [nama]} :query} :parameters}]
                         (clojure.pprint/pprint "test4")
                         {:status 200
                          :body "oke oke"})}}]]]
...
(comment
  (println "Hello World")
  (start)
  (let [student {:id 1 :nama "Bagus Jatikusuma"}
        student2 {:id 2 :nama "Iqbal"}]
    (println (:11111 student))))
